...
| Waydroid (Android)                | Yes          | 1. Launch games via `mtracker waydroid app launch {AndroidAppName}` in a terminal/app shortcut.<br>2. Make sure to call `waydroid session stop` after you're done or MiniTracker won't know when to end the session!                           |
| PCSX2 (PS2)                       | Yes          | Recommended to use PCSX2's shortcut creator and just prepend `mtracker` , otherwise just make sure the path of the game is the last argument like `mtracker pcsx2 -- 'path/to/game.iso'`                                                       |
| Dolphin (Gamecube/Wii)            | Yes          | Always use `-e '/path/to/your/.iso'` or `--exec='path/to/your/.iso` at some point when calling `dolphin-emu`, other arguments can be anywhere. It's not possible to tell apart Gamecube and Wii games, so the PlatformID is always set to Wii. |
| GOG (via Heroic)                  | No           |                                                                                                                                                                                                                                                |
